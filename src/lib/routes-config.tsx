...
import { Info, Settings, List, Feather, Users, User, Shield, Home } from "lucide-react";
...
export const routesConfig: Record<string, NestedRouteConfig> = {
  ...
  posts: {
    ...
    children: [
      {
        path: "",
        element: <div className="flex items-center justify-center h-full text-muted-foreground">Select a post to view</div>,
        title: "Posts List",
  ...
  for (const route of Object.values(routesConfig)) {
    ...
    if (route.children) {
      for (const child of route.children) {
        const fullChildPath = route.path + (child.path ? `/${child.path}` : "");
...
export function generateBreadcrumbPath(routeKey: string, params?: Record<string, string>): Array<{
  title: string;
  path: string;
  isActive: boolean;
}> {
...
export function matchRouteToConfig(pathname: string): { route: RouteConfig; key: string; params: Record<string, string> } | null {
  for (const [key, route] of Object.entries(routesConfig)) {
    ...
    if (route.children) {
      for (const child of route.children) {
        ...
        if (childMatch) {
          return { route: child, key: `${key}_child`, params: childMatch.params };
        }
      }
    }
  }
...
export function getNavigationItems(category: 'blog' | 'posts' | 'admin'): Array<{
  name: string;
  route: string;
  icon: any;
}> {
  switch (category) {
    case 'blog':
      return [
        {
          name: routesConfig.blogInfo.breadcrumbTitle || routesConfig.blogInfo.title,
          route: routesConfig.blogInfo.path,
          icon: routesConfig.blogInfo.icon,
        },
        {
          name: routesConfig.blogSettings.breadcrumbTitle || routesConfig.blogSettings.title,
    ...
    case 'posts':
      return [
        ...
        ...(routesConfig.posts.children?.filter(child => !child.hideFromBreadcrumb).map(child => ({
          name: child.breadcrumbTitle || child.title,
          route: routesConfig.posts.path + (child.path ? `/${child.path}` : ""),
          icon: child.icon,
        })) || []),
      ];
    ...
    case 'admin':
      return [
        {
          name: routesConfig.adminUsers.breadcrumbTitle || routesConfig.adminUsers.title,
          route: routesConfig.adminUsers.path,
          icon: routesConfig.adminUsers.icon,
        },
        {
          name: routesConfig.adminPermissions.breadcrumbTitle || routesConfig.adminPermissions.title,
